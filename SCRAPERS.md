...
**Cross-source deduplication**: Artsy aggregates from many auction houses and overlaps with dedicated scrapers (notably Rago). A second dedupe pass removes Artsy lots when the same artist + title + close date is already covered by a direct-house scraper.
